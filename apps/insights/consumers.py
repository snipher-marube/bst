...
class DashboardConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            handlers = {
                ...
                'layout_update': self.handle_update_layout,
    ...
    async def handle_update_layout(self, data):
        ...
        if success:
            print(f"✅ Layout saved successfully for dashboard {self.dashboard_id}")
    ...
    async def widget_deleted(self, event):
        """Handle widget deleted messages"""
        try:
            await self.send(text_data=json.dumps({
                'type': 'widget_deleted',
                'widget_id': event['widget_id']
    ...
    def get_dashboard_data(self):
        """Get dashboard data with proper serialization"""
        try:
            ...
            serializer = DashboardSerializer(dashboard, context={'request': mock_request})
            data = serializer.data
            
            # Manually convert any remaining UUIDs to strings
            data = self._convert_uuids_to_strings(data)
    ...
    async def send_dashboard_state(self):
        ...
        try:
            ...
            if data:
                # Use our custom encoder to ensure proper serialization
                message = json.dumps({
            ...
            await self.send(text_data=json.dumps({
                'type': 'error',
                'message': f'Error sending dashboard state: {str(e)}'
            }))
            ...
            with transaction.atomic():
                ...
                for widget_data in widgets:
                    ...
                    if widget_id and position:
                        ...
                        if updated:
                            updated_count += 1
                            print(f"Updated widget {widget_id} position to {position_data}")
                
                print(f"Updated {updated_count} out of {len(widgets)} widgets")
                return True
